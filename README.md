...
1. Initialize the SDK with the IBM Cloud region of your application, and optionally, your credentials:
	...
	**Note:** If you are using dedicated service, use overrideServerHost and add any of the bluemixRegion (IBM Cloud region) value.
	The IBM Cloud regions where the Push Notifications service is hosted are `PushNotifications.Region.US_SOUTH`, `PushNotifications.Region.UK`,  `PushNotifications.Region.SYDNEY`, `PushNotifications.Region.FRANKFURT` and `PushNotifications.Region.WASHINGTON`
